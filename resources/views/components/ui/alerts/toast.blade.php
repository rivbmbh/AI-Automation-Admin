...
@php
    $variantClasses = [
        'success' => 'border-green-500 bg-green-50 dark:border-green-500/30 dark:bg-green-500/15',
        'error'   => 'border-red-500 bg-red-50 dark:border-red-500/30 dark:bg-red-500/15',
        'warning' => 'border-yellow-500 bg-yellow-50 dark:border-yellow-500/30 dark:bg-yellow-500/15',
        'info'    => 'border-blue-500 bg-blue-50 dark:border-blue-500/30 dark:bg-blue-500/15',
    ];
    ...
    $borderBg = $variantClasses[$variant] ?? $variantClasses['success'];
    $iconColor = $iconColorClasses[$variant] ?? $iconColorClasses['success'];
    $icon = $icons[$variant] ?? $icons['success'];
    $resolvedTitle = $title ?? ($defaultTitles[$variant] ?? 'Notification');
...
>
    <div class="flex items-start gap-3 rounded-xl border p-4 shadow-lg {{ $borderBg }}">
        ...
        <div class="flex-1">
            ...
            @if(count($errors) > 0)
                <ul class="mt-1 list-inside list-disc space-y-0.5 text-sm text-gray-500 dark:text-gray-400">
...
<script>
    (function () {
        ...
        const duration = {{ $duration === null ? 'null' : (int) $duration }};
